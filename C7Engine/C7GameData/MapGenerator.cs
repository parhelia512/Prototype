namespace C7GameData {
	...
	public class MapGenerator {
		...
		public static GameMap GenerateMap(WorldCharacteristics wc) {
			...
			AssignTextureDetails(wc, gameMap);
		...
		private static void AssignTextureDetails(WorldCharacteristics wc, GameMap m) {
			Random rand = new Random(wc.mapSeed + 0xebac);
		...
		private static int GetTextureFileIndex(string[] neighbors, TextureFile textureFile) {
			...
			int[] cumulativeMiddleWeights = { 1, 3, 9, 27 };  // Set A: Adds {1, 2, 6, 18} per step
			int[] cumulativeEndWeights = { 2, 6, 18, 54 }; // Set B: Adds {2, 4, 12, 36} per step

			int location = 0;
			for (int i = 0; i < 4; i++) // Loop through positions N, NW, NE, Center
			{
